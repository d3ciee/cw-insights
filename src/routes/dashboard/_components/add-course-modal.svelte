<script lang="ts">
	...
	export let onSuccess: (c: TCourse & { assignedTo: string }) => void;
...
<Dialog.Root bind:open>
	<Dialog.Content class="sm:max-w-[425px]">
		<Dialog.Header>
			...
			<Dialog.Description>Create a new course by adding the following fields</Dialog.Description>
		</Dialog.Header>

		<form use:enhance={onSubmit} action="/dashboard/courses?/create" method="post">
			<div class="grid gap-4 py-4">
				...
				<div class="grid grid-cols-4 items-center gap-4">
					<Label for="headOfDepartment" class="text-right">ASSIGNED TO</Label>
					<Select.Root required>
						<Select.Trigger class="col-span-3">
						...
						<Select.Input required name="assignedToId" />
					</Select.Root>
				</div>
			</div>
			<div class="h-6">
